...
namespace PAYONEER.GPM_API
{
    public class Add_New_Forward_Email_Gmail
    {
        ...
        public Set_Acc_Result_Model Add (Account Acc, UndectedChromeDriver receive_driver)
        {
            ...
            try
            {
                ...
                add_fw_email_input.SendKeys(Acc.Forward_Email);
                Thread.Sleep(RdTimes.T_1500());
                ...
                Thread.Sleep(RdTimes.T_1000());
                int counttttttt = input_code_box.Count();
                ...
                if (check_text.Length > (Acc.Forward_Email.Length + 5))
                {
                    var account = db.Accounts.Where(x => x.ID == Acc.ID).FirstOrDefault();
                    account.Add_NewForwardEmail = true;
                    db.SaveChanges();
                    result.Status = true;
                }
            }
